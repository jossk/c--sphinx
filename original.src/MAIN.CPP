...
#ifdef OPTVARCONST
"/ORV replase variable on constant     /OIR skip repeate initializing register",
#else
"                                      /OIR skip repeate initializing register",
#endif
"",
"                          CODE GENERATION",
...
"/NW=##     disable select warning     /WE=## selected warning will be error",
...
long filesize;
	...
	if((filesize=filelength(filehandle))!=-1L){
		...
		if(filesize+outptr<MAXDATA){
			...
			puts("Symbio COM file too large.");
			exit(e_symbioerror);
		}
	}
	else{
